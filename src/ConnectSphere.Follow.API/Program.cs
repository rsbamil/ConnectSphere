...
try
{
    ...
    builder.Services.AddDbContext<FollowDbContext>(o => o.UseNpgsql(connectionString));
    ...
    builder.Services.AddHttpClient("AuthService", c => c.BaseAddress = new Uri(authServiceUrl));
    builder.Services.AddHttpClient("NotifService", c => c.BaseAddress = new Uri(notifServiceUrl));

    builder.Services.AddEndpointsApiExplorer();
    builder.Services.AddSwaggerGen(c =>
    {
        c.SwaggerDoc("v1", new OpenApiInfo { Title = "ConnectSphere Follow API", Version = "v1" });
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        while (retries > 0)
        {
            ...
            {
                db.Database.Migrate();
                Log.Information("Follow database migration applied.");
                break;
            }
            catch (Exception ex)
            {
                retries--;
                if (retries == 0) throw;
                Log.Warning("Follow database migration failed. Retrying in 5 seconds... ({Retries} attempts left)", retries);
                Thread.Sleep(5000);
            }
        }
    }
    ...
    app.UseSwagger();
    app.UseSwaggerUI(c => { c.SwaggerEndpoint("/swagger/v1/swagger.json", "Follow API v1"); c.RoutePrefix = string.Empty; });
    app.UseCors();
...
catch (Exception ex) { Log.Fatal(ex, "Follow API failed to start."); }
